<template>
    <header class="bg-gradient-to-r from-orange-600 to-yellow-500 text-white shadow-lg fixed top-0 left-0 w-full z-50">
      <div class="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <Link href="/" class="flex items-center space-x-3">
          <img
            ...
            alt="Gursha Logo"
            class="h-12 w-auto hover:opacity-90 transition duration-300"
          />
          <span class="text-2xl font-bold tracking-tight hover:text-yellow-200 transition duration-300">
            Gursha
          </span>
        </Link>
        ...
        <nav class="hidden md:flex items-center space-x-8">
          ...
          <Link href="/cart" class="relative flex items-center group">
            <svg
              class="w-6 h-6 fill-current group-hover:text-yellow-200 transition duration-300"
            ...
            <span
              ...
              class="absolute -top-2 -right-2 bg-red-600 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center group-hover:bg-red-700 transition duration-300"
            >
              {{ totalItems }}
            </span>
          </Link>
          ...
          <template v-else>
            ...
            <Link href="/my_orders" class="nav-link">My Orders</Link>
            <Link href="/user/logout" as="button" method="post" class="nav-link">Logout</Link>
            <Link href="/profile" class="flex items-center group">
              <div
                class="w-10 h-10 bg-yellow-400 text-orange-800 rounded-full flex items-center justify-center text-xl font-bold group-hover:bg-yellow-300 transition duration-300"
              >
        ...
        <button @click="toggleMenu" class="md:hidden focus:outline-none">
          <svg
            class="w-8 h-8 fill-current hover:text-yellow-200 transition duration-300"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
          >
      ...
      <transition name="slide">
        <div v-if="isMenuOpen" class="md:hidden bg-orange-600 text-white px-4 py-6">
          <div class="flex items-center justify-between mb-4">
            <!-- Logo and Restaurant Name in Mobile Menu -->
            <Link href="/" class="flex items-center space-x-3">
              <img
                src="/image/logo1.webp"
                alt="Gursha Logo"
                class="h-10 w-auto hover:opacity-90 transition duration-300"
              />
              <span class="text-xl font-bold tracking-tight hover:text-yellow-200 transition duration-300">
                Gursha
              </span>
            </Link>
            <button @click="toggleMenu" class="text-white focus:outline-none">
              <svg
                class="w-6 h-6 fill-current hover:text-yellow-200 transition duration-300"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
              >
          ...
          <Link href="/cart" class="mobile-nav-link flex items-center">
            ...
            <span
              v-if="cartItems.length > 0"
              class="ml-2 bg-red-600 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center"
            >
              {{ totalItems }}
            </span>
          </Link>
          ...
          <template v-else>
            ...
            <Link href="/my_orders" class="mobile-nav-link">My Orders</Link>
            ...
            <Link href="/profile" class="mobile-nav-link flex items-center">
              Profile
              <div
                class="ml-2 w-8 h-8 bg-yellow-400 text-orange-800 rounded-full flex items-center justify-center text-lg font-bold"
              >
                {{ auth.user.name.charAt(0).toUpperCase() }}
              </div>
            </Link>
