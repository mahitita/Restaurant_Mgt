<template>
  ...
  const { props } = usePage();
  const auth = props.auth ?? { user: null }; // Fallback if auth is undefined
  
  // Dynamic Data with safeguards
  const stats = ref([
    { label: 'Total Orders', value: props.ordersCount || 25, color: 'text-orange-600' },
    { label: 'Revenue Today', value: `$${Number(props.revenue || 1250).toFixed(2)}`, color: 'text-green-600' },
