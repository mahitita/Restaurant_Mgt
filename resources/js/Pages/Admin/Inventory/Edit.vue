<template>
      ...
      <div class="container mx-auto p-6 bg-white shadow-md rounded-lg">
        ...
        <form @submit.prevent="submit">
          ...
          <div class="flex">
            ...
            >
              {{ form.processing ? 'Updating...' : 'Update Inventory Item' }}
            </button>
            <button
              type="button"
  ...
  const form = useForm({
    ...
    unit: props.inventory.unit,
    threshold: props.inventory.threshold,
    expiry_date: props.inventory.expiry_date,
  });

  const submit = () => {
    form.put(`/admin/inventory/${props.inventory.id}`, {
      onSuccess: () => router.get('/admin/inventory'),
    });
  };
  </script>
